# shared-utilities/kafka/kafka_advanced_client.py
import json
import logging
import threading
import time
...
class KafkaSyncConsumer:
    ...
    def __init__(self,
                 ...
                 **kwargs):
        """
        Initialize sync consumer.

        Args:
            topics: List of topic names to subscribe to
            bootstrap_servers: Kafka broker addresses
            group_id: Consumer group ID
            auto_commit: If True, offsets are committed automatically
            auto_poll: If True, enables continuous polling mode
    ...
    def poll_messages(self, timeout_ms: int = 1000, max_records: int = 500) -> List[ConsumerRecord]:
        """
        Poll for messages manually (single batch).
    ...
    def consume_continuously(self, message_handler, max_empty_polls: int = 10):
        ...
        Args:
            message_handler: Function to handle each message (msg) -> bool
            max_empty_polls: Stop after this many empty polls
        """
        ...
        while not self._stop_polling and empty_poll_count < max_empty_polls:
            ...
            for message in messages:
                try:
                    ...
                    # Auto-commit individual message if enabled and manual commit mode
    ...
    def commit_message(self, message: ConsumerRecord) -> bool:
        ...
        Args:
            message: The ConsumerRecord to commit
    ...
    def commit_all_pending(self) -> bool:
        ...
        Returns:
            bool: True if all commits successful
        """
...
def create_basic_producer(bootstrap_servers: str) -> KafkaSyncProducer:
    """Create a basic producer with default settings."""
    return KafkaSyncProducer(bootstrap_servers)


def create_manual_consumer(topics: List[str],
                           bootstrap_servers: str,
                           group_id: str) -> KafkaSyncConsumer:
    """Create a consumer with manual commit and manual polling."""
...
def create_auto_consumer(topics: List[str],
                         bootstrap_servers: str,
                         group_id: str) -> KafkaSyncConsumer:
    """Create a consumer with auto-commit and continuous polling."""
    return KafkaSyncConsumer(
        topics=topics,
        bootstrap_servers=bootstrap_servers,
        group_id=group_id,
...
if __name__ == "__main__":
    ...
    def manual_processing_example():
        ...
        # Poll when you want
        messages = consumer.poll_messages(timeout_ms=5000)
    ...
    # Scenario 2: Continuous with manual commit
    def continuous_manual_commit_example():
        consumer = KafkaSyncConsumer(
            topics=["test-topic"],
            bootstrap_servers="localhost:9092",
            group_id="continuous-group",
            auto_commit=False,  # Manual commit
            auto_poll=True  # Continuous polling
        )
        ...
        consumer.consume_continuously(message_handler)
        consumer.close()
