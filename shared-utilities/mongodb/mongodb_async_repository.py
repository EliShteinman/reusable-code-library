# ============================================================================
# shared-utilities/mongodb/mongodb_async_repository.py - ASYNC CRUD ONLY
# ============================================================================
import logging
...
class MongoDBAsyncRepository:
    """
    MongoDB ASYNC CRUD repository - OPERATIONS ONLY
    Works ONLY with MongoDBAsyncClient
    """

    def __init__(self, client: AsyncMongoClient, collection_name: str):
        """
        Initialize async repository.

        Args:
            client: MongoDBAsyncClient instance
    ...
    async def create(self, document: Dict[str, Any]) -> str:
        """Create document in MongoDB (async)."""
    ...
    async def get_by_id(self, doc_id: str, id_field: str = "_id") -> Optional[Dict[str, Any]]:
    ...
    async def get_all(self, limit: int = 0, sort_by: List[tuple] = None, **filters) -> List[Dict[str, Any]]:
        """Get all documents with optional filters (async)."""
    ...
    async def update(self, doc_id: str, updates: Dict[str, Any], id_field: str = "_id") -> bool:
        ...
        result = await self.client.execute_update_one(self.collection_name, query, updates_with_meta)
    ...
    async def delete(self, doc_id: str, id_field: str = "_id") -> bool:
        ...
        result = await self.client.execute_delete_one(self.collection_name, query)
        return result.deleted_count > 0

    async def search(self, limit: int = 10, skip: int = 0, sort_by: List[tuple] = None, **filters) -> Dict[str, Any]:
        """
    ...
    async def count(self, **filters) -> int:
        ...
        query = self._build_query(**filters)
        return await self.client.execute_count_documents(self.collection_name, query)
    ...
    async def bulk_update(self, updates: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for update_doc in updates:
            try:
                ...
                result = await self.client.execute_update_one(self.collection_name, filter_query, update_data)
    ...
    async def bulk_delete(self, filters: List[Dict[str, Any]]) -> Dict[str, Any]:
        """Bulk delete documents by filters (async)."""
        success_count = 0
        error_count = 0

        for filter_query in filters:
            try:
                result = await self.client.execute_delete_one(self.collection_name, filter_query)
    ...
    async def aggregate(self, pipeline: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        Perform aggregation on the collection (async).
        ...
        """
        try:
            # Note: Need to implement execute_aggregate in async client
    ...
    async def to_dataframe(self, **filters) -> pd.DataFrame:
        ...
        try:
            # Get all results
            documents = await self.get_all(**filters)
    ...
    async def create_text_index(self, fields: List[str]):
        """Create text index for full-text search (async)."""
        try:
            collection = self.client.get_collection(self.collection_name)
    ...
    async def drop_collection(self):
        """Drop the entire collection (async)."""
        try:
            collection = self.client.get_collection(self.collection_name)
